...
const getSopVersion = (id, version) => getRequest(`sop/version?id=${id}&version=${version}`);
...
const updateSopStep = (id, data) => patchRequest(`sop/step?id=${id}`, data, getToken());
const deleteSopStep = (id) => deleteRequest(`sop/step?id=${id}`, getToken());
