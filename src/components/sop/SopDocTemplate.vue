        ...
        <div class="overflow-x-auto" style="margin: 0 auto;">
            ...
            <div class="w-[297mm] relative mb-10">
                ...
                <svg class="absolute inset-0 w-full h-full pointer-events-none z-0">
                    <arrow-connector v-for="(connection, index) in connections" :idarrow="index"
                        idcontainer="sop-container" :key="`${connection.from}-${connection.to}`"
                        :connection="connection" @mounted="handleArrowMounted" />
                </svg>
            </div>
        </div>
    </div>
